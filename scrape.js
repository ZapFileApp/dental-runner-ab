...
(async () => {
  ...
  const outputDir = 'Canada_cities';
  if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });

  for (const city of cities) {
    const searchQueries = [
      `Dentist in ${city} ${STATE_NAME}`,
      `Dental clinic in ${city} ${STATE_NAME}`,
      `Best dentist in ${city} ${STATE_NAME}`,
      `Dental office in ${city} ${STATE_NAME}`,
      `Top dental clinics in ${city} ${STATE_NAME}`
    ];

    const filePath = `${outputDir}/${city.replace(/\s+/g, '_')}_Dental.csv`;
